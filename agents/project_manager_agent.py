...
class ProjectManagerAgent:
    ...
    def _get_development_plan(self, idea: str) -> Union[List[dict], None]:
        ...
        **CRITICAL RULE: You MUST generate configurations for *both* local 
        development and production.**
        * **The `.env` file (for local dev):** The `DATABASE_URL` MUST use `localhost` 
            (e.g., `postgresql+psycopg2://myuser:mypassword@localhost/mydatabase`).
        * **The `docker-compose.yml` file (for prod):** The `app` service's 
            `environment` section MUST override the `DATABASE_URL` to use the 
            service name `db` (e.g., `DATABASE_URL: "postgresql+psycopg2://myuser:mypassword@db/mydatabase"`).
